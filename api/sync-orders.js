...
function address(a = {}, s = {}) {
	...
	return (
		...
		.filter(Boolean)
		.join(', ') || [fallback.address_1, fallback.address_2, fallback.city, fallback.state, fallback.postcode,
			fallback.country
		]
		.filter(Boolean)
		.join(', ')
	...
	try {
		...
		for (const site of sites || []) {
			...
			for (let page = 1; page <= MAX_PAGES_PER_SITE; page++) {
				...
				params.set('consumer_key', site.consumer_key)
				params.set('consumer_secret', site.consumer_secret)
				...
				for (const o of orders) {
					...
					const {
						...
						.from('orders')
						.upsert(row, {
							onConflict: 'site_id,wc_order_id'
						})
						.select('id')
						.single()
